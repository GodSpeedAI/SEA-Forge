...
2. **Authority-Before-Effect**:
   - CJ06's AUTHORITY gate result: `FAIL` (approval decision actor/verdict observed on the real IPC transcript before any downstream settlement).
3. **Stale Precondition & Recovery**:
   - CJ08 exercised a real stale-precondition commit rejection and re-preflight recovery. Result: `FAIL`.
