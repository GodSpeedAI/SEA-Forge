...
| The `.sea` seed parses and validates through domainforge-core 0.13.0 | Evidence-backed | M9 gate: `load_validate` green on both bundled models via the M0 library adapter | `domainforge 0.13.0` CLI (workspace build): `validate --format human` ⇒ "Validation succeeded: 0 violations total"; `parse --ast --format json` ⇒ exit 0; `project --format kg` ⇒ Turtle emitted. Run 2026-07-14 against seed sha256 `2ea06fc9c59d28fac9b9d47f18c4787b2ffb740b0527513a70556cf91dcbffc5` | M9 must re-prove through the in-process `load_validate` path (not the CLI) and pin the seed hash in the release realization |
