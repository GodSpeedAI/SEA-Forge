...
| T13.2 | 5 mixed sandboxed and agent episodes, server semaphore=2 | server-owned scheduler respects one shared cap; ledger replay reproduces ordering | partial: semaphore cap proven for concurrent agent delegations; mixed sandboxed+agent and ledger-replay ordering pending |
| T13.3 | cancel one of three in flight; restart during cancel or ACP approval | durable control/permission state yields exactly one terminal outcome or ledger-linked successor; siblings settle normally | partial: cancel-one-of-three proven (siblings settle accepted, cancelled settles rejected); restart/successor-episode scenario pending |
